...
import Reservationsalle from "../modals/Reservationsalle";


export const Reservationsalles = () =>{
    ...
    const columns = [
        ...
        { field: 'salle', headerName: 'Salles', width: 250, editable: true },
        { field: 'libelle', headerName: 'Libelle', width: 300, editable: true },
        { field: 'dateceremonie', headerName: 'Date Ceremonie', width: 250, editable: true },
        ...
        renderCell : (params)=>{
          return(
            <>
              ...
              <button className='btn' onClick={()=> deleteReservesalles(params.row._id)}><DeleteIcon color="secondary"/>Supprimer</button>
              </>
          )
        }
      },
    ...
    useEffect(()=>{
      axios.get('http://localhost:8000/reservesalle')
      .then(res =>{
        setRows(res.data.reservationsalles)
      })
    },[enregistrement])
    const deleteReservesalles = async (id) =>{
      await axios.delete(`http://localhost:8000/reservesalle/${id}`)
        .then((res)=>{
          setRows(res.data);
        })        
    };
  ...
  return (
    <div className='dashboard container-fluid'>
                ...
                <div className="page-title mt-3 ml-4">
                    ...
                    <div className="container-fluid">
                        <ol className="breadcrumb">
                            <li className="active"><i className="fa fa-dashboard"></i> Dashboard / Reservation Salles</li>
                        </ol>
                    </div>                            
                </div>
            </div>
        ...
        <Popup
            title="Entrer les informations actuelles de Reservation Salles"
            openPopup={openPopupModification}
            setOpenPopup={setOpenPopupModification}
            >  
                <Reservationsalle reservationsalle={dataModi} />
        </Popup>
        <Popup
        ...
        >  
            <Reservationsalle data={dataToSave}/>
        </Popup>
    
        <Popup
        title="Enregistrement de Reservation Salles"
        openPopup={openPopupForm}
        setOpenPopup={setOpenPopupForm}
        >  
            <Reservationsalle reservationsalle={undefined} setEnregistrement={setEnregistrement}/>
        </Popup>    
